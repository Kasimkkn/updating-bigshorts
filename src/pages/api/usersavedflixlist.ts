...
// import { FLixSavedListRequest } from '@/services/usersavedflixlist';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<VideoSavedResponse>) {
    if (req.method === 'POST') {
        ...
        try {
            ...
            const response = await fetch(API_ENDPOINTS.saved.flix, {
                method: 'POST',
                headers: {
            ...
            console.error('User FLIX Saved failed:', error);
            res.status(500).json({
