...
import { getPostListNew, GetPostListRequest } from "@/services/auth/getpostlist";
...
const useFetchPosts = (
    ...
    const fetchPosts = useCallback(async (userId: string) => {
        ...
        const requestData: GetPostListRequest = {
            isForYou: "1",
            isForAll: "1",
            limit: 10,
        ...
        try {
            const response: PostlistResponse = await getPostListNew(requestData);
            if (response.isSuccess && response.data) {
                setPosts((prevPosts) => [...prevPosts, ...response.data]);
                pageRef.current += 1;
            } else {
    ...
    }, [usersPost, snipId, fetchPostList, fetchPosts, isHydrated]);
